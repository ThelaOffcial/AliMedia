/**
 * Server-side copy of src/utils/commentModeration.ts. Deliberately kept
 * logic-identical (same word lists, same regex construction) so a comment
 * can't be flagged client-side but pass server-side or vice versa. If you
 * update the client version, mirror the change here too.
 */
 ...
 * unrelated word (e.g. "sex" inside "Sussex", "rape" inside "therapist").
 * Falls back to the unbounded pattern if lookbehind isn't supported.
 */
function withWordBoundary(pattern) {
  try {
    return new RegExp(
 ...
 *  for detection only — never used for the displayed text. */
function collapseStretchedLetters(input) {
  return input.replace(/([a-zA-Z\u0D80-\u0DFF])\1{2,}/g, '$1');
}

function flexibleWordRe(word, optionalVowels = true) {
...
const SKELETON_RES = [
  ...
  {
    re: withWordBoundary('p[\\W_0-9@#$%*]*o+[\\W_0-9@#$%*]*n+[\\W_0-9@#$%*]*a+[\\W_0-9@#$%*]*y+[\\W_0-9@#$%*]*a+'),
    label: 'ponnaya',
  },
  {
    re: withWordBoundary('h[\\W_0-9@#$%*]*u+[\\W_0-9@#$%*]*t+[\\W_0-9@#$%*]*h*[\\W_0-9@#$%*]*t+[\\W_0-9@#$%*]*h*[\\W_0-9@#$%*]*a+'),
    label: 'huththa',
  },
];
...
/**
 * @param {string} raw
 * @returns {{ allowed: boolean, flagged: boolean, reason?: string, matchedTerms: string[], sanitized: string }}
 */
function moderateCommentText(raw) {
  const text = (raw || '').trim();
  if (!text) {
  ...
  // Whole-word matches only, so words like "therapist"/"Sussex"/"cockpit"
  // are not falsely flagged for containing a banned word as a substring.
...
module.exports = { moderateCommentText };
